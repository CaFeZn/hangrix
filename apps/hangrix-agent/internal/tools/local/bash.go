...
// HasRunningJobs reports how many async work items are still keeping the
// wake alive. That includes:
//   - background bash tasks that have not reached `done`
//   - pending sleep timers that have not fired yet
//   - terminal notifications already buffered on notifyCh but not yet
//     drained by the runtime loop
//
// The last bullet closes a subtle race at the idle-grace boundary:
// without counting a just-fired notification, a sleep timer could delete
// itself from b.timers, the transport could observe "0 running jobs" and
// EOF the wake, and the loop could exit before the queued notification was
// processed. Treating buffered notifications as still-running work keeps
// the wake alive until the loop has a chance to react.
