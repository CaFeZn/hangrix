...
// workflowListRunsTool resolves the repo ID from /me and calls
// GET /repos/{repoID}/workflow-runs?commit_sha=<sha>.
type workflowListRunsTool struct {
	name        string
	description string
	schema      map[string]any
	client      *Client
...
func (t *workflowListRunsTool) Name() string           { return t.name }
func (t *workflowListRunsTool) Description() string    { return t.description }
func (t *workflowListRunsTool) Schema() map[string]any { return t.schema }

func (t *workflowListRunsTool) Call(ctx context.Context, args json.RawMessage) (any, error) {
	if len(args) == 0 || strings.TrimSpace(string(args)) == "" {
		args = json.RawMessage(`{}`)
	}
	repoID, err := t.client.repoID(ctx)
	if err != nil {
		return nil, fmt.Errorf("workflow_list_runs: resolve repo id: %w", err)
	}
	...
	if err := json.Unmarshal(args, &req); err != nil {
		return nil, fmt.Errorf("workflow_list_runs: parse arguments: %w", err)
	}
	if req.CommitSHA == "" {
		return nil, fmt.Errorf("workflow_list_runs: commit_sha is required")
	}

	path := fmt.Sprintf("/repos/%d/workflow-runs", repoID)
	query := "commit_sha=" + req.CommitSHA

	var resp v1Singleton
	err = t.client.doV1(ctx, http.MethodGet, path, query, nil, false, &resp)
	if err != nil {
		msg := err.Error()
		if strings.HasPrefix(msg, "{") {
			var parsed any
...
// workflowDownloadLogTool streams a job log to a local file and returns the path.
type workflowDownloadLogTool struct {
	name        string
	description string
	schema      map[string]any
	client      *Client
...
func (t *workflowDownloadLogTool) Name() string           { return t.name }
func (t *workflowDownloadLogTool) Description() string    { return t.description }
func (t *workflowDownloadLogTool) Schema() map[string]any { return t.schema }

func (t *workflowDownloadLogTool) Call(ctx context.Context, args json.RawMessage) (any, error) {
	if len(args) == 0 || strings.TrimSpace(string(args)) == "" {
		args = json.RawMessage(`{}`)
	}
	...
	if err := json.Unmarshal(args, &req); err != nil {
		return nil, fmt.Errorf("workflow_download_job_log: parse arguments: %w", err)
	}
	if req.JobRunID == 0 {
		return nil, fmt.Errorf("workflow_download_job_log: job_run_id is required")
	}

	if err := os.MkdirAll("/tmp/hangrix-logs", 0o755); err != nil {
		return nil, fmt.Errorf("workflow_download_job_log: create log dir: %w", err)
	}

	path := fmt.Sprintf("/tmp/hangrix-logs/job-%d.txt", req.JobRunID)
	logURL := t.client.baseURL + "/api/v1/workflow-jobs/" + strconv.FormatInt(req.JobRunID, 10) + "/log"

	httpReq, err := http.NewRequestWithContext(ctx, http.MethodGet, logURL, nil)
	if err != nil {
		return nil, fmt.Errorf("workflow_download_job_log: build request: %w", err)
	}
	httpReq.Header.Set("Authorization", "Bearer "+t.client.token)
	httpReq.Header.Set("Accept", "text/plain")

	resp, err := t.client.http.Do(httpReq)
	if err != nil {
		return nil, fmt.Errorf("workflow_download_job_log: download: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(io.LimitReader(resp.Body, 4096))
		return nil, fmt.Errorf("workflow_download_job_log: server returned %d: %s", resp.StatusCode, truncate(body, 256))
	}

	f, err := os.OpenFile(path, os.O_CREATE|os.O_TRUNC|os.O_WRONLY, 0o644)
	if err != nil {
		return nil, fmt.Errorf("workflow_download_job_log: open file: %w", err)
	}
	defer f.Close()

	written, err := io.Copy(f, resp.Body)
	if err != nil {
		return nil, fmt.Errorf("workflow_download_job_log: write file: %w", err)
	}
...
func All(client *Client, async local.AsyncLifecycle, readOnly bool) []local.Tool {
	...
	defs := []toolDef{
		// ---- read-only tools ----
		{name: "issue_read", description: "Read the current issue's metadata, comments, and timeline events. The response includes a `checkers` array: each item represents a job-level CI check (name, status, conclusion, job_run_id, run_id, workflow_name) anchored to the issue branch's head commit. An empty array means no checks have run yet.", kind: "get", path: "/issues/current"},
		{name: "issue_comment_read", description: "Read the full body of a single comment by its id. Only comments on the current session's issue or its direct parent issue are accessible — cross-issue lookups return 'not found'. Returns the full body (no truncation).",
			kind: "get", path: "/issues/current/comments/{comment_id}", pathParams: []string{"comment_id"},
			schema: objectSchema(map[string]any{"comment_id": intProp("The comment id to read (required). Must belong to the current session's issue or its direct parent.")}, []string{"comment_id"}),
		},
		...
		{name: "issue_checks", description: "List the latest state of each CI check on the issue's head commit. Returns a `checkers` array of job-level CI statuses (name, status, conclusion, job_run_id, run_id, workflow_name, event_name). Prefer the `checkers` field already injected into issue_read and contribution_read responses; use this only when you need a standalone poll.",
		...
		{name: "contribution_read", description: "Read one contribution: metadata (id, agent_role, actor with kind/id/display_name/role_key, ref_name, status, mergeable, merge_mode, head_sha, diff stats), review status (verdict, required reviewers still pending), a checkout_hint with the branch ref and issue branch names so you can fetch the contribution branch locally and inspect its diff with git, and a `checkers` array (job-level CI statuses anchored to the contribution's head commit). Use the id from contribution_list.",
		...
		{name: "issue_depends_add", description: "Add a dependency edge: make the current issue depend on another issue. The target must be in the same repo. Cyclic dependencies are rejected with a structured error.",
			kind: "post", path: "/issues/current/dependencies", write: true,
			schema: objectSchema(map[string]any{
				"depends_on": intProp("Issue number this issue depends on (required). Must be in the same repo; the edge must not create a cycle."),
			}, []string{"depends_on"}),
		},
		{name: "issue_depends_remove", description: "Remove a dependency edge from the current issue, so it no longer depends on the given issue. The edge must exist.",
			kind: "delete", path: "/issues/current/dependencies/{depends_on_number}", pathParams: []string{"depends_on_number"}, expect204: true, write: true,
			schema: objectSchema(map[string]any{
	...
	for _, d := range defs {
		...
		case "workflow_list_runs":
			out = append(out, &workflowListRunsTool{
				name:        d.name,
				description: d.description,
				schema:      schema,
				client:      client,
		...
		case "workflow_log":
			out = append(out, &workflowDownloadLogTool{
				name:        d.name,
				description: d.description,
				schema:      schema,
				client:      client,
							...
							"items": map[string]any{
								...
								"properties": map[string]any{
									"label": map[string]any{
										"type":        "string",
										"description": "Option label shown to the user. 1-1000 characters.",
										"minLength":   1,
										"maxLength":   1000,
									},
								},
