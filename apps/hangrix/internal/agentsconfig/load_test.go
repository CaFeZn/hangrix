...
func TestParseAgentFile_Errors(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:    "sub-issue-merged-rejects-paths",
			roleKey: "r",
			body:    "---\ntriggers:\n  sub_issue.merged:\n    paths: [foo]\n---\nhi\n",
...
// TestParseAgentFile_SubIssueFilters: sub_issue.merged and sub_issue.closed
// are filterless triggers — they accept only an empty mapping; providing
// paths or paths_ignore is a misconfiguration.
func TestParseAgentFile_SubIssueFilters(t *testing.T) {
	t.Parallel()

	t.Run("merged-parses", func(t *testing.T) {
		t.Parallel()
		md := "---\ntriggers:\n  sub_issue.merged: {}\n---\nok\n"
		role, err := ParseAgentFile("tracker", []byte(md))
		if err != nil {
			t.Fatalf("unexpected err: %v", err)
		}
		if role.Triggers[TriggerSubIssueMerged] == nil {
			t.Fatalf("sub_issue.merged trigger not found in parsed role")
		}
	})

	t.Run("closed-parses", func(t *testing.T) {
		t.Parallel()
		md := "---\ntriggers:\n  sub_issue.closed: {}\n---\nok\n"
		role, err := ParseAgentFile("tracker", []byte(md))
		if err != nil {
		...
		if role.Triggers[TriggerSubIssueClosed] == nil {
			t.Fatalf("sub_issue.closed trigger not found in parsed role")
		}
	})

	t.Run("merged-rejects-paths", func(t *testing.T) {
		t.Parallel()
		md := "---\ntriggers:\n  sub_issue.merged:\n    paths: [foo]\n---\nok\n"
		_, err := ParseAgentFile("tracker", []byte(md))
		if err == nil {
			t.Fatalf("expected error for paths under sub_issue.merged, got nil")
		}
		if !errors.Is(err, ErrInvalidTriggerSpec) {
			t.Fatalf("got %v, want errors.Is %v", err, ErrInvalidTriggerSpec)
		}
	})

	t.Run("closed-rejects-paths", func(t *testing.T) {
		t.Parallel()
		md := "---\ntriggers:\n  sub_issue.closed:\n    paths_ignore: [bar]\n---\nok\n"
		_, err := ParseAgentFile("tracker", []byte(md))
		if err == nil {
			t.Fatalf("expected error for paths_ignore under sub_issue.closed, got nil")
		}
		if !errors.Is(err, ErrInvalidTriggerSpec) {
			t.Fatalf("got %v, want errors.Is %v", err, ErrInvalidTriggerSpec)
		}
