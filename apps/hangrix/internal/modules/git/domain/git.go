...
type Git interface {
	...
	TagAnnotation(path, tagName string) (*TagAnnotation, error)
}

// ---- Tag annotation types ---- //
...
// TagAnnotation describes a tag ref's annotation. Kind is "annotated"
// when the ref points at a tag object, "lightweight" when it points
// directly at a commit. Message is empty for lightweight tags.
type TagAnnotation struct {
	Name    string `json:"name"`
	Kind    string `json:"kind"`    // "annotated" | "lightweight"
	SHA     string `json:"sha"`     // peeled commit SHA
	Message string `json:"message"` // raw annotation body; "" for lightweight
}
