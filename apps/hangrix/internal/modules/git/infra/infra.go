...
// TagAnnotation reads the annotation of refs/tags/tagName. Annotated
// tags return Kind=TagKindAnnotated with the message; lightweight tags
// return Kind=TagKindLightweight with an empty message. Missing tags
// return ErrRefNotFound.
func (g *GoGit) TagAnnotation(path, tagName string) (*domain.TagAnnotation, error) {
	repo, err := openRepo(path)
	if err != nil {
		return nil, err
	}
	ref, err := repo.Reference(plumbing.NewTagReferenceName(tagName), false)
	if err != nil {
		if errors.Is(err, plumbing.ErrReferenceNotFound) {
			return nil, domain.ErrRefNotFound
		}
		return nil, fmt.Errorf("tag annotation: lookup: %w", err)
	}
	obj, err := repo.Object(plumbing.AnyObject, ref.Hash())
	if err != nil {
		return nil, fmt.Errorf("tag annotation: object: %w", err)
	}
	switch v := obj.(type) {
	case *object.Tag:
		commit, err := v.Commit()
		if err != nil {
			return nil, fmt.Errorf("tag annotation: peel: %w", err)
		}
		return &domain.TagAnnotation{
			Name:    tagName,
			Kind:    domain.TagKindAnnotated,
	...
	case *object.Commit:
		return &domain.TagAnnotation{
			Name:    tagName,
			Kind:    domain.TagKindLightweight,
			SHA:     v.Hash.String(),
			Message: "",
		}, nil
	default:
