...
const listMembersByProviderID = `-- name: ListMembersByProviderID :many
SELECT
    m.id, m.group_id, m.provider_id,
    p.name AS provider_name,
    m.model, m.priority,
...
type ListMembersByProviderIDRow struct {
	ID                int64
	GroupID           int64
	ProviderID        int64
	ProviderName      string
...
func (q *Queries) ListMembersByProviderID(ctx context.Context, providerID int64) ([]ListMembersByProviderIDRow, error) {
	rows, err := q.db.Query(ctx, listMembersByProviderID, providerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListMembersByProviderIDRow{}
	for rows.Next() {
		var i ListMembersByProviderIDRow
		if err := rows.Scan(
			&i.ID,
			&i.GroupID,
			&i.ProviderID,
