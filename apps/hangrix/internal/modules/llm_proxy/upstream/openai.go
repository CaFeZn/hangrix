...
//
// Store + Include carry the stateless-reasoning contract documented at
// https://developers.openai.com/api/docs/guides/reasoning :
//   - Store: pointer-to-false tells OpenAI not to keep its own copy of
//     this response. The proxy is fully stateless — we never use
//     previous_response_id to chain — so leaving the upstream's default
//     store:true on creates a state-machine conflict (server thinks it
//     owns the reasoning chain, we keep replaying full input) that
//     manifests after a few rounds as `status:"completed", output:null`
//     responses with non-zero usage.output_tokens.
//   - Include: opt-in for encrypted_content on reasoning items in the
//     response. Without this, the server returns reasoning items with
//     summary_text but EMPTY encrypted_content; we round-trip those
//     empty blobs as input, and after a few rounds the model loses
//     reasoning state. Only meaningful alongside store:false.
...
func buildResponsesAPIRequestBody(req *Request) openaiRequest {
	...
	// Always run stateless against the OpenAI Responses API. We keep our
	// own audit history in agent_session_messages and never chain via
	// previous_response_id — so opting out of server-side storage avoids
	// the dual-state conflict that intermittently produces empty
	// `output:null` responses after a few reasoning rounds (see Store
	// commentary above). For reasoning models specifically we also need
	// to ask for `encrypted_content` so the items we replay carry the
	// blob the model needs to resume its chain-of-thought; without the
	// include opt-in we'd be replaying empty encrypted_content fields.
	storeOff := false
	body.Store = &storeOff
	if req.ReasoningEffort != "" {
