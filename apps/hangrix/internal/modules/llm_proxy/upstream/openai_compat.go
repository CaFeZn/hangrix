...
func inputItemsToChatMessages(items []InputItem, instructions string) []chatMessage {
	...
	flushPending()
	return stripTrailingAssistantChat(out)
}

// stripTrailingAssistantChat removes trailing assistant chat messages
// from the output slice so the conversation always ends with a user or
// tool message. Some Chat Completions providers (DeepSeek, among others)
// reject conversations ending with an assistant message as "assistant
// message prefill" — the Responses-API input shape permits it (history
// can be sent verbatim), but Chat Completions requires the last message
// to be from the user.
func stripTrailingAssistantChat(msgs []chatMessage) []chatMessage {
