...
type Request struct {
	...
	// Canonical values "minimal" / "low" / "medium" / "high" / "xhigh"
	// / "max" drive the Anthropic adapter's `output_config.effort`
	// (Claude 4.6+ canonical) or the DEPRECATED `thinking.budget_tokens`
	// translation (legacy — rejected by Opus 4.7+/4.8, deprecated on
	// 4.6), and analogous handling in OpenAI o-series. Any other
	// non-empty string is passed through verbatim so newer upstream
	// enum values work without an adapter change. Adapters without a
	// native reasoning knob ignore the field.
	ReasoningEffort string

	// Thinking selects the Anthropic extended-thinking mode: "adaptive"
	// (Claude 4.6+ canonical, RECOMMENDED — the only mode Opus 4.7/4.8
	// accepts), "enabled" (DEPRECATED — legacy `thinking: {type: enabled,
	// budget_tokens: N}` derived from ReasoningEffort; rejected by 4.7+),
	// or "disabled" (omit `thinking`, temperature intact).
	// Empty means "field absent on the wire" — the Anthropic adapter
	// then omits `thinking` and the upstream applies its model default
	// (no extended thinking on Claude 4.7/4.8). Non-Anthropic adapters
	// ignore this field.
	Thinking string
