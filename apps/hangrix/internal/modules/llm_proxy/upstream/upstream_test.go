...
func TestOpenAIRespondTranslatesInputItems(t *testing.T) {
	...
	// Stateless-reasoning contract: store must be explicitly false so
	// OpenAI doesn't try to chain via its own server-side copy of the
	// response (we keep our own history and don't use previous_response_id).
	// And include must opt into encrypted_content so the reasoning items
	// the upstream returns carry a non-empty blob we can replay next turn
	// — without this opt-in the items come back with empty
	// encrypted_content and reasoning chains intermittently break after
	// a few rounds.
	if store, ok := seen["store"].(bool); !ok || store != false {
		t.Errorf("store=%v (type=%T), want false", seen["store"], seen["store"])
	}
	include, _ := seen["include"].([]any)
	if len(include) != 1 || include[0] != "reasoning.encrypted_content" {
		t.Errorf("include=%v, want [reasoning.encrypted_content]", seen["include"])
	}
...
// TestOpenAIRespondNonReasoningOmitsInclude verifies that the
// `include: ["reasoning.encrypted_content"]` opt-in only fires when
// the request actually asks for reasoning. For a plain non-reasoning
// model call we still want stateless (`store:false`) but `include`
// should be absent so the upstream doesn't reject an unknown
// inclusion key on a non-reasoning model.
func TestOpenAIRespondNonReasoningOmitsInclude(t *testing.T) {
	var seen map[string]any
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		_ = json.Unmarshal(body, &seen)
	...
	if store, ok := seen["store"].(bool); !ok || store != false {
		t.Errorf("store=%v, want false (always stateless)", seen["store"])
	}
...
// TestOpenAICompatStripsTrailingAssistant verifies that when the input
// ends with an assistant message (no following user message), the
// outgoing chat-completions request drops that trailing assistant so
// providers that reject "assistant prefill" (DeepSeek, etc.) don't 400.
func TestOpenAICompatStripsTrailingAssistant(t *testing.T) {
	var seen map[string]any
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		_ = json.Unmarshal(body, &seen)
	...
	req := &upstream.Request{
		Model: "m", APIKey: "x", BaseURL: srv.URL, Client: srv.Client(),
		Input: []upstream.InputItem{
			{Kind: upstream.KindMessage, Role: "user", Text: "hello"},
			{Kind: upstream.KindMessage, Role: "assistant", Text: "hi there"},
			// No following user message — the conversation ends with assistant.
		},
	}
	if _, err := upstream.NewOpenAICompat().Respond(context.Background(), req); err != nil {
		t.Fatalf("respond: %v", err)
	...
	msgs, _ := seen["messages"].([]any)
	if len(msgs) != 1 {
		t.Fatalf("expected 1 message (only user), got %d: %v", len(msgs), msgs)
	}
	user, _ := msgs[0].(map[string]any)
	if user["role"] != "user" {
		t.Errorf("last message role = %v, want user", user["role"])
	}
}

// TestAnthropicStripsTrailingAssistant verifies the same guard on the
// Anthropic side: a conversation ending with an assistant message has its
// trailing assistant dropped before dispatch.
func TestAnthropicStripsTrailingAssistant(t *testing.T) {
	var seen map[string]any
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		_ = json.Unmarshal(body, &seen)
	...
	if msg["role"] != "user" {
		t.Errorf("last message role = %v, want user", msg["role"])
	}
}

// ---- Anthropic adapter ----

// TestAnthropicRespondLegacyThinkingEnabled verifies the legacy
// `thinking: {type: enabled, budget_tokens: N}` path (DEPRECATED —
// rejected by Opus 4.7/4.8, deprecated on 4.6; kept only for older
// Claudes). When Request.Thinking == "enabled", the adapter maps
// ReasoningEffort to a budget, bumps max_tokens above it, and drops
// temperature.
func TestAnthropicRespondLegacyThinkingEnabled(t *testing.T) {
	cases := []struct {
		effort           string
		wantBudget       int
	...
	for _, c := range cases {
		t.Run(c.effort, func(t *testing.T) {
			...
			// Legacy enabled path does NOT also send output_config.effort
			// (the two are alternative ways to say the same thing).
			if _, hasOC := seen["output_config"]; hasOC {
				t.Errorf("output_config must NOT be sent on legacy enabled path; body=%v", seen)
			}
...
// TestAnthropicRespondAdaptiveThinking verifies the Claude 4.6+ shape:
// `thinking: {type: "adaptive"}` plus `output_config: {effort: ...}`,
// no budget_tokens, no temperature.
func TestAnthropicRespondAdaptiveThinking(t *testing.T) {
	var seen map[string]any
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		_ = json.Unmarshal(body, &seen)
	...
	if thinking == nil || thinking["type"] != "adaptive" {
		t.Fatalf("thinking=%v, want {type: adaptive}", thinking)
	}
	if _, hasBudget := thinking["budget_tokens"]; hasBudget {
		t.Errorf("budget_tokens must NOT be sent on adaptive path; body=%v", seen)
	}
	...
	if _, hasTemp := seen["temperature"]; hasTemp {
		t.Errorf("temperature must be dropped on adaptive path; body=%v", seen)
	}
}

// TestAnthropicRespondEffortOnlyEmitsOutputConfig verifies that an
// effort-only request (no Thinking) emits `output_config.effort` but
// no `thinking` field — the model decides on its own whether to
// reason, but stays within the effort guidance.
func TestAnthropicRespondEffortOnlyEmitsOutputConfig(t *testing.T) {
	var seen map[string]any
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		_ = json.Unmarshal(body, &seen)
	...
	if _, hasThinking := seen["thinking"]; hasThinking {
		t.Errorf("thinking must NOT be set when Thinking is unset; body=%v", seen)
	}
	oc, _ := seen["output_config"].(map[string]any)
	if oc == nil || oc["effort"] != "medium" {
		t.Errorf("output_config=%v, want {effort: medium}", oc)
	}
}

// TestAnthropicRespondDisabledKeepsTemperature verifies that disabled
// thinking emits no thinking block AND keeps temperature intact —
// disabled thinking is normal (non-extended-thinking) mode where
// temperature is supported on all Claude models.
func TestAnthropicRespondDisabledKeepsTemperature(t *testing.T) {
	var seen map[string]any
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		_ = json.Unmarshal(body, &seen)
	...
	// No thinking block should be sent.
	if _, hasThinking := seen["thinking"]; hasThinking {
		t.Errorf("thinking must NOT be set when Thinking=disabled; body=%v", seen)
	}
	// Temperature must be preserved — disabled thinking is normal mode.
	gotTemp, hasTemp := seen["temperature"].(float64)
	if !hasTemp {
		t.Errorf("temperature must be preserved on disabled path; body=%v", seen)
	}
	if gotTemp != 0.7 {
...
func TestAnthropicRoundTripsThinkingOnSubsequentTurn(t *testing.T) {
	...
	req := &upstream.Request{
		Model: "m", APIKey: "x", BaseURL: srv.URL, Client: srv.Client(),
		Thinking: "enabled", ReasoningEffort: "medium", // legacy enabled path keeps thinking on
		Input: []upstream.InputItem{
			{Kind: upstream.KindMessage, Role: "user", Text: "q1"},
