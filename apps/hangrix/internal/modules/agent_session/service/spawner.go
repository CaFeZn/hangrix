...
// AgentRunCreator is the spawner's narrow view of the workflow service.
// Defined here (not in workflow/domain) so the spawner doesn't carry the
// workflow service's wider surface; the concrete *workflowsvc.Service
...
type Spawner struct {
	...
	// per session id so spawnRole/rewakeRole can hand the literal value
	// to workflow.CreateAgentRun without re-decrypting the sealed blob.
	// Lifetime is one Spawner method call; cleared after the workflow
	// handoff. Not concurrency-safe (OnTrigger is the only caller).
	plaintextTokens map[int64]string
}
...
// dispatchAgentRun is the spawner→workflow handoff that replaces the
// runner.session pickup path. Given an already-persisted agent_session
// row, build the AgentRunSpec from the same role-snapshot inputs the
// runner side used to receive on Task.Env, and ask the workflow service
// to create the _agent.wake workflow_run. The runner picks the
// resulting workflow_job up via /tasks (the same channel that delivers
// user workflow_jobs); its WorkflowJobDriver starts a fresh container
// that execs /opt/hangrix/hangrix-agent — at which point the agent
// authenticates with the session token it found in HANGRIX_SESSION_TOKEN
// and pulls events from /api/agent/sessions/{id}/inputs.
//
// Errors here leave the inputs queue + message log already populated
// (callers always enqueue cause first), so a retry on the next trigger
// just re-enters the same path; alreadyForCause dedup prevents
// duplicate input frames.
func (s *Spawner) dispatchAgentRun(
	ctx context.Context,
	...
	sessionID int64,
	sessionToken string,
	effective *agentsconfig.LLMConfig,
	addendum string,
) error {
	if s.workflow == nil {
		// Defensive — module.go always wires workflow. A nil here
		// means a misconfigured test container; fail loudly instead
		// of silently skipping the agent dispatch.
		return fmt.Errorf("workflow dispatcher not wired")
	}
	...
	var platformToolsJSON string
	if patternsJSON, err := json.Marshal(role.ToolPatterns); err == nil {
		platformToolsJSON = string(patternsJSON)
	}
	spec := workflowsvc.AgentRunSpec{
		Repo: workflowsvc.Ref{
			ID:            hostRepo.ID,
			OwnerName:     hostRepo.OwnerName,
			Name:          hostRepo.Name,
		...
		// env into the agent container; the agent's prompt assembler
		// splices it under baseline.md. Empty when the role has no
		// addendum. Pre-cutover the runner orchestrator bind-mounted
		// this as a file; that path no longer exists since the agent
		// runs as a workflow_job step, so we plumb it via env.
		RolePrompt:   addendum,
		TriggerActor: triggerActorFromInput(in),
	}
...
// llmModel / llmEffort / llmThinking are small extractors that tolerate
// a nil resolved-LLM block (the spawner's resolveLLM returns nil only when
// neither host nor role declared any llm, which the spawnRole guard
// already rejects).
func llmModel(c *agentsconfig.LLMConfig) string {
	if c == nil {
		return ""
	}
	return c.Model
...
func llmEffort(c *agentsconfig.LLMConfig) string {
	if c == nil || c.ReasoningEffort == nil {
		return ""
	}
	return *c.ReasoningEffort
}

func llmThinking(c *agentsconfig.LLMConfig) string {
	if c == nil || c.Thinking == nil {
		return ""
	}
	return *c.Thinking
}
...
// ref the workflow run records on its trigger_actor_* columns. Falls
// back to a zero-value Ref (workflow service derives a System actor) when
// the caller didn't carry one.
func triggerActorFromInput(in domain.TriggerInput) actor.Ref {
	if in.Actor != nil && !in.Actor.IsZero() {
		return *in.Actor
	}
	if in.ActorID > 0 {
		return actor.UserRef(in.ActorID, "")
	}
	return actor.Ref{}
}
...
func (s *Spawner) OnTrigger(ctx context.Context, in domain.TriggerInput) ([]domain.SpawnedSession, error) {
	...
	// Silence gate: when the repo is silenced, only enqueue-onto-live
	// is allowed (the agent will process events when silence lifts).
	// New spawns and rewakes are suppressed so the runner doesn't
	// pick up the session while the repo is silent.
	var silenceErr error
	if s.silenceGate != nil {
		silenceErr = s.silenceGate.CheckRepo(ctx, in.RepoID)
	}
	...
	for _, roleKey := range keys {
		...
		// Silence gate: when the repo is silenced, only enqueue-onto-live
		// is permitted — new spawns and rewakes are suppressed.
		// Enqueuing onto a live session is safe because the agent is
		// already running and will pick up the event when silence lifts.
		if silenceErr != nil {
			existing, hasExisting := existingByRole[roleKey]
			if hasExisting && existing.Status != runnerdomain.SessionStatusArchived && isLiveStatus(existing.Status) {
			...
			// Suppressed: no live session, and we can't spawn or rewake.
			s.recordSpawnError(ctx, in, roleKey, silenceErr)
			out = append(out, domain.SpawnedSession{
				SessionID: 0,
				RoleKey:   roleKey,
				Action:    domain.SpawnActionSuppressed,
			})
			continue
		}

		// Non-archived sessions (idle / succeeded / cancelled / failed)
		// are rewoken so the session row and its identity (token, message
		// history, container workspace) are preserved across triggers.
		// Previously, failed sessions were excluded and spawned fresh
		// rows — this broke session continuity and was a bug (#267).
		// The orphaned-inputs concern (cross-cause replay) is benign:
		// any unconsumed inputs from a prior failed rewake are events
		// the agent should still process, and the alreadyForCause guard
		// already prevents duplicate causes from being enqueued.
		if existing, hasExisting := existingByRole[roleKey]; hasExisting &&
			...
			// Session identity (token, message history, container
			// workspace) is preserved across rewake.
...
// cancelled / failed) back to 'pending' and enqueues a cause frame so
...
func (s *Spawner) rewakeRole(ctx context.Context, in domain.TriggerInput, existing *runnerdomain.AgentSession) (domain.SpawnedSession, error) {
	...
	// Enqueue the cause event and the audit message BEFORE creating the
	// workflow_run. Same race rationale as spawnRole: the agent's first
	// /inputs poll must find the cause frame already queued, otherwise
	// the EOF-on-empty exit fires before the agent does any work.
	...
	// token. Always call ResumeSession — not just when the token rotated
	// — so the state machine stays coherent: dispatchAgentRun's
	// MarkSessionRunning expects pending/claimed/running, and the
	// agent's POST /idle MarkSessionIdle expects claimed/running. Before
	// this change a token-preserved rewake of a 'failed' row left the
	// status at 'failed' and every downstream transition silently
	// no-op'd (the agent ran but the row never advanced) — which is
	// exactly the symptom users hit when a stopped session was
	// re-triggered by an event.
	if err := s.runner.ResumeSession(ctx, existing.ID, tok); err != nil {
		_ = s.runner.MarkSessionTerminal(ctx, existing.ID, runnerdomain.SessionStatusFailed, nil, "rewake resume-session failed: "+err.Error())
		return domain.SpawnedSession{}, fmt.Errorf("resume session on rewake: %w", err)
	}

	// Resolve the host repo + config the agent will boot against. Same
...
// resolveSpawnSnapshot re-resolves the host repo, host yaml, role config,
// and head SHA at rewake time. Kept separate from spawnRole's inline
// resolution so the rewake path can use the same logic without
// duplicating the trigger-input plumbing.
func (s *Spawner) resolveSpawnSnapshot(ctx context.Context, repoID int64, roleKey string) (*repodomain.Repo, *agentsconfig.HostConfig, *agentsconfig.Role, string, error) {
	hostRepo, err := s.repos.GetByID(ctx, repoID)
	if err != nil {
		return nil, nil, nil, "", fmt.Errorf("host repo lookup: %w", err)
	}
	hostFs, err := s.storage.ResolvePath(hostRepo.OwnerName, hostRepo.Name)
	if err != nil {
	...
	if hostCfg == nil {
		return nil, nil, nil, "", fmt.Errorf("host yaml missing at rewake")
	}
...
// new event without a fresh container spin-up.
//
// We intentionally do NOT mirror the event into agent_session_messages.
// Mirroring was causing ordering violations on rewake: if the agent was
// mid-turn (assistant(tool_calls) emitted but tool_result not yet
// persisted), the mirrored event row would land between them in the
// audit log, and FetchHistory on rewake would replay them in the
// invalid order assistant → event → tool_result. The Anthropic / OpenAI
// API layer rejects this because tool_result must immediately follow
// the assistant message that contains the matching tool_use.
//
// Instead, the event is delivered exclusively through the inputs queue.
// On rewake, FetchHistory replays agent-generated messages only (no
// mid-turn events interleaved), then StreamInputs delivers the event
// frame and the agent folds it into context via applyInboxItem.
// trimTrailingEventItems already removes trailing events from history
// because those events will be redelivered from inputs — this change
// eliminates the case where events land inside an active tool chain
// where trimming can't reach them.
...
func (s *Spawner) spawnRole(
	...
	// Order matters: enqueue cause + persist message BEFORE creating the
	// workflow_run. The runner claims workflow_jobs immediately on /tasks
	// pickup, so a workflow_run created before the input is queued would
	// race the agent boot — the agent's first /inputs poll could return
	// empty and trigger an immediate EOF-exit, dropping the cause frame.
	...
	if err := s.dispatchAgentRun(ctx, hostRepo, hostCfg, role, roleKey, repoSHA, in, sess.ID, plaintext, effective, addendum); err != nil {
		// Workflow create failed — the session row + cause frame are
		// already persisted. The next trigger on this (role, cause) is
		// blocked by alreadyForCause dedupe, so a retry would have to
		// come from a NEW cause; the current session row will then sit
		// idle until reaped. Mark it failed so the operator sees the
		// cause in the admin UI instead of a silently-stuck "pending".
		_ = s.runner.MarkSessionTerminal(ctx, sess.ID, runnerdomain.SessionStatusFailed, nil, "agent workflow dispatch failed: "+err.Error())
		return domain.SpawnedSession{}, fmt.Errorf("dispatch agent run: %w", err)
	}
...
// and constructs a user actor. When both are empty/zero, the system actor
// (id=1) is used — this prevents FK violations on agent_sessions.
// created_by_actor_id which is NOT NULL REFERENCES actors(id).
func (s *Spawner) resolveTriggerActor(ctx context.Context, in domain.TriggerInput) int64 {
	if s.actorResolver == nil {
		// Without the actor module, we can't resolve or fall back.
		// Return 0 — the caller (spawnRole) must handle this.
		return 0
	}
	...
	// Neither Actor nor ActorID provided — fall back to the system
	// actor so the session row's FK is always valid.
	return s.systemActorID()
}

// systemActorID returns the id of the system actor, or 0 when the
// resolver is not wired (tests). The system actor is always id=1.
// Goes through From() to ensure the actor row exists and its ActorID
// is populated; System() alone may not have ActorID filled in.
func (s *Spawner) systemActorID() int64 {
	if s.actorResolver == nil {
		return 0
	}
	sys := s.actorResolver.System()
	if !sys.IsZero() {
		resolved, err := s.actorResolver.From(context.Background(), sys)
...
func (s *Spawner) isDirectInvoke(in domain.TriggerInput) bool {
	...
	switch in.CauseKind {
	case domain.CauseKindQuestionnaireAnswered,
		domain.CauseKindManual:
		return true
	}
	return false
}
...
func buildRoleSnapshot(role *agentsconfig.Role, host *agentsconfig.HostConfig, addendum, model string, effective *agentsconfig.LLMConfig, resolvedImage string) ([]byte, error) {
	...
	if effective != nil {
		...
		if effective.Thinking != nil {
			snap.LLMThinking = *effective.Thinking
		}
	}
...
// `reasoning_effort: ""` without it being indistinguishable from "field
// omitted".
func resolveLLM(role *agentsconfig.Role, host *agentsconfig.HostConfig) *agentsconfig.LLMConfig {
	...
	if perRole != nil {
		...
		if perRole.Thinking != nil {
			out.Thinking = perRole.Thinking
		}
	}
	return out
}
...
//
// Baseline payload keys are repo_id, issue_number, actor_id, cause_kind,
// and cause_id. For cause event issue.opened, callers may also attach
// issue_title and issue_body in TriggerInput.Payload (see docs/runner-protocol.md
// "Cause event — issue.opened payload"). Per-trigger details (comment body,
// push delta, vote value, etc.) ride in.Payload and are merged into
// payload without rewriting the wire shape.
