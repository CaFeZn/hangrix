...
// stubWorkflow records every CreateAgentRun call so tests can assert
// the spawner dispatched (or didn't) for each role. The returned run/
// job pair is deterministic — just enough shape for assertions.
...
func (w *stubWorkflow) CreateAgentRun(_ context.Context, spec workflowsvc.AgentRunSpec) (*workflowdomain.WorkflowRun, []*workflowdomain.WorkflowJobRun, error) {
	w.mu.Lock()
	defer w.mu.Unlock()
	if err, ok := w.failOn[spec.SessionID]; ok {
		return nil, nil, err
	}
	w.calls = append(w.calls, spec)
...
// Calls returns the recorded specs in dispatch order.
func (w *stubWorkflow) Calls() []workflowsvc.AgentRunSpec {
	w.mu.Lock()
	defer w.mu.Unlock()
...
// production does. No-op when the session row isn't tracked here.
func (r *stubRunnerRepo) MarkSessionRunning(_ context.Context, id int64) error {
	for _, s := range r.sessions {
		if s.ID == id {
			s.Status = runnerdomain.SessionStatusRunning
			return nil
		}
	}
	return nil
