...
WHERE i.repo_id = $1
  ...
  AND (NOT $3::BOOLEAN OR i.parent_id IS NULL)
`
...
func (q *Queries) CountIssues(ctx context.Context, arg CountIssuesParams) (int64, error) {
	row := q.db.QueryRow(ctx, countIssues, arg.RepoID, arg.State, arg.RootsOnly)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
const listAllDescendantIssues = `-- name: ListAllDescendantIssues :many
...
type ListAllDescendantIssuesParams struct {
	RepoID  int64
	RootIds []int64
}

type ListAllDescendantIssuesRow struct {
	ID                 int64
	RepoID             int64
	Number             int64
	ActorID            int64
...
func (q *Queries) ListAllDescendantIssues(ctx context.Context, arg ListAllDescendantIssuesParams) ([]ListAllDescendantIssuesRow, error) {
	rows, err := q.db.Query(ctx, listAllDescendantIssues, arg.RepoID, arg.RootIds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListAllDescendantIssuesRow{}
	for rows.Next() {
		var i ListAllDescendantIssuesRow
		if err := rows.Scan(
			&i.ID,
			&i.RepoID,
			&i.Number,
...
const listMentionedIssueIDs = `-- name: ListMentionedIssueIDs :many
...
WHERE c.issue_id = ANY($1::BIGINT[])
  ...
  AND i.state = 'open'
  AND c.body ~* ('(^|[^A-Za-z0-9_])@' || $2::TEXT || '([^A-Za-z0-9_]|$)')
`

type ListMentionedIssueIDsParams struct {
	IssueIds []int64
	Username string
}
...
func (q *Queries) ListMentionedIssueIDs(ctx context.Context, arg ListMentionedIssueIDsParams) ([]int64, error) {
	rows, err := q.db.Query(ctx, listMentionedIssueIDs, arg.IssueIds, arg.Username)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const listPendingQuestionnaireIssueIDs = `-- name: ListPendingQuestionnaireIssueIDs :many

SELECT DISTINCT q.issue_id
FROM questionnaires q
LEFT JOIN questionnaire_answers a
  ON a.questionnaire_id = q.id AND a.actor_id = $1
WHERE q.issue_id = ANY($2::BIGINT[])
  AND q.status = 'open'
  AND a.id IS NULL
`

type ListPendingQuestionnaireIssueIDsParams struct {
	ActorID  int64
	IssueIds []int64
}
...
func (q *Queries) ListPendingQuestionnaireIssueIDs(ctx context.Context, arg ListPendingQuestionnaireIssueIDsParams) ([]int64, error) {
	rows, err := q.db.Query(ctx, listPendingQuestionnaireIssueIDs, arg.ActorID, arg.IssueIds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const listQueuedAgentRunIssueIDs = `-- name: ListQueuedAgentRunIssueIDs :many
...
func (q *Queries) ListQueuedAgentRunIssueIDs(ctx context.Context, issueIds []int64) ([]int64, error) {
	rows, err := q.db.Query(ctx, listQueuedAgentRunIssueIDs, issueIds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
