...
// an agent actor; otherwise it falls back to the system actor. displayName is
// the user's username: it is persisted as the actor's display_name so the read
// path (which derives author_name from actors.display_name) can show the author.
func (s *PostgresStore) resolvedActor(ctx context.Context, authorID int64, displayName, agentRole string) (int64, error) {
	switch {
	case authorID > 0:
		a, err := s.actorStore.EnsureUser(ctx, authorID, displayName)
		if err != nil {
...
func (s *PostgresStore) Create(ctx context.Context, repoID, authorID int64, authorName, title, body, baseBranch string, agentRole string, parentID, parentNumber int64) (*domain.Issue, error) {
	...
	actorID, err := s.resolvedActor(ctx, authorID, authorName, agentRole)
	if err != nil {
...
func (s *PostgresStore) ListDescendants(ctx context.Context, repoID int64, rootIDs []int64) ([]*domain.Issue, error) {
	if len(rootIDs) == 0 {
		return nil, nil
	}
	rows, err := s.q.ListAllDescendantIssues(ctx, issuedb.ListAllDescendantIssuesParams{
		RepoID:  repoID,
		RootIds: rootIDs,
	})
	if err != nil {
	...
	out := make([]*domain.Issue, 0, len(rows))
	for _, r := range rows {
		out = append(out, issueFromDescendant(r))
	}
	return out, nil
}

func (s *PostgresStore) LoadIndicators(ctx context.Context, issueIDs []int64, callerUserID int64, callerUsername string) (map[int64]domain.IssueIndicators, error) {
	result := make(map[int64]domain.IssueIndicators, len(issueIDs))
	for _, id := range issueIDs {
		result[id] = domain.IssueIndicators{IssueID: id}
	}
	if callerUserID == 0 || len(issueIDs) == 0 {
		return result, nil
	}
	// Resolve the user to an actor_id for questionnaire answer lookups.
	a, err := s.actorStore.EnsureUser(ctx, callerUserID, callerUsername)
	if err != nil {
		return nil, fmt.Errorf("resolve user actor: %w", err)
	}
	callerActorID := a.ActorID

	// Batch 1: pending questionnaire
	pendingIDs, err := s.q.ListPendingQuestionnaireIssueIDs(ctx, issuedb.ListPendingQuestionnaireIssueIDsParams{
		ActorID:  callerActorID,
		IssueIds: issueIDs,
	})
	if err != nil {
		return nil, fmt.Errorf("list pending questionnaire ids: %w", err)
	}
	for _, id := range pendingIDs {
		r := result[id]
		r.HasPendingQuestionnaire = true
		result[id] = r
	}
	// Batch 2: agent mentions
	mentionedIDs, err := s.q.ListMentionedIssueIDs(ctx, issuedb.ListMentionedIssueIDsParams{
		IssueIds: issueIDs,
		Username: callerUsername,
	})
	if err != nil {
		return nil, fmt.Errorf("list mentioned issue ids: %w", err)
	}
...
func issueFromDescendant(r issuedb.ListAllDescendantIssuesRow) *domain.Issue {
	iss := &domain.Issue{
		ID:             r.ID,
		RepoID:         r.RepoID,
		Number:         r.Number,
...
// the actor; agent_role is implicitly the empty string. sourceIssueID is 0
// for regular comments; it is set for cross-issue comments.
func (s *PostgresStore) CreateComment(ctx context.Context, issueID, authorID int64, authorName, body, filePath string, line int, sourceIssueID int64) (*domain.Comment, error) {
	actorID, err := s.resolvedActor(ctx, authorID, authorName, "")
	if err != nil {
		return nil, fmt.Errorf("comment: resolve actor: %w", err)
	}
	var src pgtype.Int4
	if sourceIssueID != 0 {
		src = pgtype.Int4{Int32: int32(sourceIssueID), Valid: true}
	}
...
// sourceIssueID is 0 for regular comments; it is set for cross-issue comments.
func (s *PostgresStore) CreateAgentComment(ctx context.Context, issueID int64, agentRole, body, filePath string, line int, sourceIssueID int64) (*domain.Comment, error) {
...
func (s *PostgresStore) ListComments(ctx context.Context, issueID int64) ([]*domain.Comment, error) {
	...
	for _, r := range rows {
		out = append(out, &domain.Comment{
			...
			FilePath:      r.FilePath,
			Line:          int(r.Line),
			SourceIssueID: int64(r.SourceIssueID),
			CreatedAt:     r.CreatedAt.Time,
			UpdatedAt:     r.UpdatedAt.Time,
		})
	}
...
func (s *PostgresStore) CreateEvent(ctx context.Context, issueID int64, kind domain.EventKind, payload []byte, actorUserID int64, actorName string) (*domain.Event, error) {
	actorID, err := s.resolvedActor(ctx, actorUserID, actorName, "")
	if err != nil {
