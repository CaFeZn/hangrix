...
// actor_* columns first, falling back to legacy AuthorID/AgentRole.
//
// SourceIssueID is set only for cross-issue comments (posted via
// issue_comment_cross) and records the issue number the comment originated
// from. It is 0 for regular comments on the current issue.
type Comment struct {
	ID            int64
	IssueID       int64
	AuthorID      int64
...
// ErrClosedIssueImmutable is raised when a caller attempts to reopen a
// closed issue — the closed state is permanent and irreversible (issue #276).
var ErrClosedIssueImmutable = errors.New("this issue is closed and cannot be reopened")

// CanTransitionState is the single-source-of-truth rule engine for issue state
// transitions. It returns nil when the transition is allowed, or a typed
// sentinel error explaining why it is blocked.
//
// Invariants (locked to product spec for issue #276):
//   - merged → *          : ErrInvalidState (merged is terminal through PATCH)
//   - closed → open       : ErrClosedIssueImmutable (closed is permanent)
//   - closed → closed     : nil (no-op; handler short-circuits)
//   - open → closed       : nil (sub-issue gate applied by handler after)
//   - open → open         : nil (no-op)
//   - * → merged          : ErrInvalidState (must go through POST /merge)
func CanTransitionState(from, to State) error {
	if from == StateMerged || to == StateMerged {
		return ErrInvalidState
	}
	if from == StateClosed && to == StateOpen {
		return ErrClosedIssueImmutable
	}
	return nil
}

// IssueIndicators is a lightweight per-issue aggregation computed in batch
// for the list view. Only populated when ListIssuesWithIndicators is called;
// other read paths never include it.
type IssueIndicators struct {
	IssueID                 int64
...
type ListFilter struct {
	...
	Offset    int32
	Limit     int32
	RootsOnly bool // when true, only root issues (parent_id IS NULL); used for tree view
}
...
type Store interface {
	...
	// ListDescendants returns every transitive descendant (any state) of the
	// given root issue IDs, ordered (depth ASC, number ASC). The result is a
	// flat list keyed by parent_id; the frontend assembles the tree in O(N).
	ListDescendants(ctx context.Context, repoID int64, rootIDs []int64) ([]*Issue, error)
	// LoadIndicators computes indicator flags for the given issue set from the
	// caller's perspective. callerUserID=0 means no caller — all flags false.
	// callerUsername is used for mention matching (substring search in agent comments).
	LoadIndicators(ctx context.Context, issueIDs []int64, callerUserID int64, callerUsername string) (map[int64]IssueIndicators, error)
	...
	CreateComment(ctx context.Context, issueID, authorID int64, authorName, body, filePath string, line int, sourceIssueID int64) (*Comment, error)
	...
	// sourceIssueID is set only for cross-issue comments; 0 for regular.
	CreateAgentComment(ctx context.Context, issueID int64, agentRole, body, filePath string, line int, sourceIssueID int64) (*Comment, error)
