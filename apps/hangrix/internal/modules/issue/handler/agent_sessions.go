...
func (h *Handler) resetAgentSession(w http.ResponseWriter, r *http.Request) {
	sid, ok := h.resolveSession(w, r, true)
	if !ok {
		return
	}
	...
	newSID, err := h.controller.Reset(r.Context(), sid)
	if err != nil {
		switch {
		case errors.Is(err, runnerdomain.ErrSessionNotFound):
			httpx.WriteError(w, http.StatusNotFound, "agent session not found")
		case errors.Is(err, agentsessiondomain.ErrNotResettable),
			errors.Is(err, agentsessiondomain.ErrSessionLive):
			httpx.WriteError(w, http.StatusConflict, "session is not in a resettable state")
		default:
			httpx.WriteError(w, http.StatusInternalServerError, err.Error())
		}
		return
	}
	httpx.WriteJSON(w, http.StatusCreated, map[string]any{"new_session_id": newSID})
}
