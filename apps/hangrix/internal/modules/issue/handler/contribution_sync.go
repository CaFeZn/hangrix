...
func (h *Handler) fireContributionPushed(ctx context.Context, repo *repodomain.Repo, iss *domain.Issue, c *domain.Contribution, headSHA string, changedPaths []string) {
	...
	// Resolve the contributing agent as the triggering actor (new path),
	// with the issue author's user ID as legacy fallback. Mirrors
	// fireCommitPushed → resolveCommitPushedActor.
	triggerActor := h.resolveContributionPushedActor(ctx, c.AgentRole)
...
// resolveContributionPushedActor resolves the push actor for contribution
// pushes. Uses the contributor's agent role as the primary identity;
// returns nil when resolution fails — the spawner then falls back to
// the legacy ActorID (issue author).
func (h *Handler) resolveContributionPushedActor(ctx context.Context, agentRole string) *actor.Ref {
	if h.actorResolver == nil || agentRole == "" {
		return nil
	}
	resolved, err := h.actorResolver.From(ctx, actor.AgentRef(agentRole))
	if err != nil {
		log.Printf("issue: resolve contribution-pushed actor agent:%s: %v", agentRole, err)
		return nil
	}
	return resolved
}

// dispatchContributionPush fans a contribution.push workflow trigger.
func (h *Handler) dispatchContributionPush(ctx context.Context, repo *repodomain.Repo, iss *domain.Issue, c *domain.Contribution, headSHA string, changedPaths []string) {
	if h.workflowDispatcher == nil {
		return
	}
	h.workflowDispatcher.DispatchContributionPush(ctx, workflowdomain.Ref{
		ID:            repo.ID,
		Name:          repo.Name,
		DefaultBranch: repo.DefaultBranch,
		OwnerName:     repo.OwnerName,
	}, workflowdomain.ContributionPushInput{
