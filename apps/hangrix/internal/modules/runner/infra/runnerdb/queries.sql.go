...
SELECT id, runner_id, repo_id, issue_number, status, role, model, agent_image, working_branch, base_branch, host_addendum, env, session_token_prefix, session_token_hash, session_token_sealed, session_token_revoked_at, exit_code, error_message, created_at, claimed_at, started_at, ended_at, repo_sha, role_key, cause_kind, cause_id, role_config, container_id, container_last_used_at, container_cleanup_pending, container_stop_pending, container_stopped_at, running_jobs, created_by_actor_id, actor_id FROM agent_sessions
...
SELECT id, runner_id, repo_id, issue_number, status, role, model, agent_image, working_branch, base_branch, host_addendum, env, session_token_prefix, session_token_hash, session_token_sealed, session_token_revoked_at, exit_code, error_message, created_at, claimed_at, started_at, ended_at, repo_sha, role_key, cause_kind, cause_id, role_config, container_id, container_last_used_at, container_cleanup_pending, container_stop_pending, container_stopped_at, running_jobs, created_by_actor_id, actor_id FROM agent_sessions WHERE id = $1
`
...
SELECT id, runner_id, repo_id, issue_number, status, role, model, agent_image, working_branch, base_branch, host_addendum, env, session_token_prefix, session_token_hash, session_token_sealed, session_token_revoked_at, exit_code, error_message, created_at, claimed_at, started_at, ended_at, repo_sha, role_key, cause_kind, cause_id, role_config, container_id, container_last_used_at, container_cleanup_pending, container_stop_pending, container_stopped_at, running_jobs, created_by_actor_id, actor_id FROM agent_sessions WHERE session_token_prefix = $1
`
...
const listStaleRunningSessions = `-- name: ListStaleRunningSessions :many
WITH latest_llm AS (
...
SELECT
    s.id, s.runner_id, s.repo_id, s.issue_number, s.status, s.role, s.model, s.agent_image, s.working_branch, s.base_branch, s.host_addendum, s.env, s.session_token_prefix, s.session_token_hash, s.session_token_sealed, s.session_token_revoked_at, s.exit_code, s.error_message, s.created_at, s.claimed_at, s.started_at, s.ended_at, s.repo_sha, s.role_key, s.cause_kind, s.cause_id, s.role_config, s.container_id, s.container_last_used_at, s.container_cleanup_pending, s.container_stop_pending, s.container_stopped_at, s.running_jobs, s.created_by_actor_id, s.actor_id,
    GREATEST(
...
type ListStaleRunningSessionsParams struct {
	Threshold pgtype.Interval
	Lim       int32
}

type ListStaleRunningSessionsRow struct {
	ID                      int64
	RunnerID                pgtype.Int8
	RepoID                  pgtype.Int8
	IssueNumber             pgtype.Int4
...
func (q *Queries) ListStaleRunningSessions(ctx context.Context, arg ListStaleRunningSessionsParams) ([]ListStaleRunningSessionsRow, error) {
	rows, err := q.db.Query(ctx, listStaleRunningSessions, arg.Threshold, arg.Lim)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListStaleRunningSessionsRow{}
	for rows.Next() {
		var i ListStaleRunningSessionsRow
		if err := rows.Scan(
			&i.ID,
			&i.RunnerID,
			&i.RepoID,
