...
-- 'pending' is accepted alongside 'claimed' so the spawner can move a
-- freshly-created session straight to 'running' post-CreateAgentRun
-- (the runner no longer claims sessions; agent runs ride workflow_jobs
-- now). 'running' stays in the set so the call is idempotent on a
-- session that's already running (rewake replays).
UPDATE agent_sessions
SET status = 'running', started_at = COALESCE(started_at, NOW())
WHERE id = sqlc.arg('id') AND status IN ('pending', 'claimed', 'running');
