...
type Handler struct {
	...
	// silenceGate blocks git pushes from agent sessions when the repo
	// is silenced. Nil when the repo_silence module is not loaded.
	silenceGate silencedomain.SilenceGate
	// silenceStore reads the current silence state to include in the
	// public repo detail view. Nil when the repo_silence module is not loaded.
	silenceStore silencedomain.Store
...
type HandlerDeps struct {
	...
	Gate issuegatedomain.IssueActivityGate
	// SilenceGate blocks git pushes from agent sessions when the repo
	// is silenced. Optional — repo handler works without it.
	SilenceGate silencedomain.SilenceGate
	// SilenceStore reads the current silence state for the public repo
	// detail view. Optional — repo handler works without it.
	SilenceStore silencedomain.Store
	Middleware   authdomain.Middleware
...
func NewHandler(deps *HandlerDeps) *Handler {
	return &Handler{
		...
		silenceStore:    deps.SilenceStore,
...
type publicRepo struct {
	...
	Silence          *silenceDTO `json:"silence,omitempty"`
	CreatedAt        time.Time   `json:"created_at"`
	UpdatedAt        time.Time   `json:"updated_at"`
}

// silenceDTO mirrors the key fields of the repo_silence State for the
// public repo detail response.
...
func (h *Handler) getOne(w http.ResponseWriter, r *http.Request) {
	...
	// Attach silence state when the repo_silence module is loaded.
	if h.silenceStore != nil {
		state, err := h.silenceStore.GetState(r.Context(), repo.ID)
		if err == nil && state != nil && state.Active {
			pr.Silence = &silenceDTO{
				Active:         state.Active,
				Source:         state.Source,
...
func (h *Handler) transfer(w http.ResponseWriter, r *http.Request) {
	...
	case orgdomain.OwnerKindOrg:
		...
		if (!isMember || role != orgdomain.RoleOwner) && caller.Role != userdomain.RoleAdmin {
			httpx.WriteError(w, http.StatusForbidden, "You must be an organization owner to perform this action")
			return
		}
	}
