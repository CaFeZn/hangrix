...
func (h *Handler) gitReceivePack(w http.ResponseWriter, r *http.Request) {
	...
	// Silence gate: agent session pushes are blocked when the repo
	// is silenced and the session has no active override. Human
	// callers (cookie/pat/password) are not gated by silence.
	if caller != nil && caller.authMethod == "session" && caller.session != nil && caller.session.RepoID != nil && h.silenceGate != nil {
		if serr := h.silenceGate.CheckSession(r.Context(), caller.session.ID, *caller.session.RepoID); serr != nil {
			var silenced *silencedomain.ErrRepoSilenced
			if errors.As(serr, &silenced) {
				http.Error(w, silenced.Error(), http.StatusLocked)
				return
			}
			http.Error(w, "internal error checking silence state", http.StatusInternalServerError)
			return
		}
	}
