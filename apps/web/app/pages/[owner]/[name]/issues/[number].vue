...
import SilenceBanner from '@/components/silence/SilenceBanner.vue'
import { useSilence } from '@/composables/useSilence'
...
const { state: silence } = useSilence(() => owner.value, () => name.value)
...
<Tabs v-model="tab">
                  ...
                  <CardContent class="p-0">
                    ...
                    <div class="px-4 py-3 text-sm">
                      <FoldableBody
                        ...
                        :owner="owner"
                        :name="name"
                        :source-issue-id="it.data.source_issue_id"
                      />
                    </div>
                  </CardContent>
                </Card>
  ...
  <Button
              v-if="issue.state === 'open' && (issue.author_id === user?.id || issue.actor?.user_id === user?.id || canManage)"
              variant="outline"
              class="w-full"
              :disabled="stateBusy || subIssueBlocked"
              @click="closeIssue"
            >
              {{ t('issue.close') }}
            </Button>
  <p v-else-if="issue.state === 'closed'" class="text-xs text-muted-foreground px-1">{{ t('issue.closedFinal') }}</p>
