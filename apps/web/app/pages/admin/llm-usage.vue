...
function collectErrorParts(value: unknown, out: string[], depth = 0) {
  if (depth > 3 || out.length >= 4 || value == null) return
  ...
  if (Array.isArray(value)) {
    for (const item of value) {
      collectErrorParts(item, out, depth + 1)
  ...
  for (const key of ['message', 'error', 'detail', 'type', 'code']) {
    if (!(key in record)) continue
    collectErrorParts(record[key], out, depth + 1)
    if (out.length >= 4) break
  }
}

function formatStatusDetail(raw: string | undefined | null): string {
  if (!raw) return ''
  ...
  try {
    const parsed = JSON.parse(raw)
    const parts: string[] = []
    collectErrorParts(parsed, parts)
    summary = parts.length > 0 ? parts.join(' · ') : JSON.stringify(parsed)
  } catch {
        ...
        <Table>
          ...
          <TableBody>
            <TableRow v-for="r in rows" :key="r.id">
              ...
              <TableCell>
                <Badge
                  :variant="statusVariant(r.status_code, r.error_message)"
                  :title="r.error_message ? formatBody(r.error_message) : String(r.status_code)"
                  class="max-w-[22rem] align-middle"
                >
                  <span class="block max-w-full truncate">
                    {{ r.error_message ? formatStatusDetail(r.error_message) : r.status_code }}
                  </span>
                </Badge>
              </TableCell>
