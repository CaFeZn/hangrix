...
const resetBusy = ref<Set<number>>(new Set())
...
function setBusy(set: Set<number>, id: number, on: boolean) {
  ...
  if (set === resetBusy.value) resetBusy.value = new Set(set)
...
function canReset(status: string): boolean {
  return ['running', 'idle', 'succeeded', 'cancelled', 'failed'].includes(status)
}

async function resetSession(s: AgentSession) {
  const msg = s.status === 'running'
    ? t('agentSessions.actions.resetRunningConfirm')
    : t('agentSessions.actions.resetConfirm')
  if (!confirm(msg)) return

  setBusy(resetBusy.value, s.session_id, true)
  try {
    const resp = await $fetch<{ new_session_id: number }>(
      `${baseUrl.value}/${s.session_id}/reset`,
      { method: 'POST' },
    )
    await refresh()
    // Auto-select the new session
    selectedId.value = resp.new_session_id
  } catch (e: unknown) {
    const errMsg = (e as { data?: { error?: string } })?.data?.error ?? t('agentSessions.actions.resetFailed')
    error.value = String(errMsg)
  } finally {
    setBusy(resetBusy.value, s.session_id, false)
  }
}
</script>
        ...
        <CardContent class="space-y-4 p-4">
          ...
          <header class="flex flex-wrap items-start justify-between gap-3 border-b pb-3">
            ...
            <div class="flex flex-wrap items-center gap-2">
              ...
              <!-- Resume: for failed or terminal-but-not-archived sessions. -->
              <Button
                v-if="selected.status === 'failed' || selected.status === 'idle' || selected.status === 'succeeded' || selected.status === 'cancelled'"
                variant="outline"
                size="sm"
                class="h-7 px-2 text-xs"
              ...
              <!-- Reset: visible for all non-terminal statuses that are not pending/claimed/archived. -->
              <Button
                v-if="canReset(selected.status)"
                variant="outline"
                size="sm"
                class="h-7 px-2 text-xs"
                :disabled="busyIn(resetBusy, selected.session_id)"
                @click="resetSession(selected)"
              >
                <RefreshCw class="size-3.5" />
                {{ busyIn(resetBusy, selected.session_id) ? t('agentSessions.actions.resetting') : t('agentSessions.actions.reset') }}
              </Button>
              <!-- Delete: allowed on anything not live and not archived. -->
              <Button
                v-if="!isLive(selected.status) && selected.status !== 'archived'"
                variant="ghost"
                size="sm"
