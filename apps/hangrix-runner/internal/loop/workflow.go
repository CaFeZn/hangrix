...
type WorkflowJobDriver struct {
	...
	// AgentBinaryDir is the host directory containing the embedded
	// hangrix-agent binary. When non-empty, jobs declaring a volume named
	// "hangrix-agent" get a read-only bind mount of this directory; see
	// orchestratorVolumes. Set by loop.go from the same agentbin.Extract
	// result the session driver uses.
...
func (d *WorkflowJobDriver) Run(ctx context.Context, job *client.WorkflowJob) error {
	...
	// 6. Resolve phases and register them on the platform so the UI
	//    shows phase entries immediately.
	type phaseDecl struct {
		kind     string
		seq      int32
		imageRef string
	}
	var phases []phaseDecl
	if buildSpec != nil {
		phases = append(phases, phaseDecl{kind: "image_build", seq: 0, imageRef: image})
	} else {
		phases = append(phases, phaseDecl{kind: "image_pull", seq: 0, imageRef: image})
	}
	phases = append(phases, phaseDecl{kind: "container_start", seq: 1, imageRef: image})

	for _, pd := range phases {
		req := client.RegisterPhaseRequest{
			Phase:         pd.kind,
			SequenceIndex: pd.seq,
			ImageRef:      pd.imageRef,
		}
		if err := d.Client.RegisterWorkflowJobPhase(ctx, job.JobRunID, req); err != nil {
			log.Printf("workflow job %d: register phase %s: %v", job.JobRunID, pd.kind, err)
			// Non-fatal: the phase endpoints are idempotent and the
			// running/mark calls will implicitly create if needed.
		}
	}
	...
	for _, pd := range phases {
		...
		if err := d.Client.MarkWorkflowJobPhaseRunning(ctx, job.JobRunID, pd.kind); err != nil {
			log.Printf("workflow job %d: mark phase %s running: %v", job.JobRunID, pd.kind, err)
		}

		events := make(chan orchestrator.PhaseEvent, 64)
		phaseCtx, phaseCancel := context.WithTimeout(ctx, phaseTimeout)

		var opErr error
		skipped := false

		// Run the phase operation in a goroutine; the main goroutine
		// forwards events as log lines.
		go func() {
			defer close(events)
			switch pd.kind {
			case "image_pull":
				skipped, opErr = d.Orchestrator.EnsureImagePulled(phaseCtx, image, events)
			case "image_build":
				skipped, opErr = d.Orchestrator.BuildImage(phaseCtx, image, buildSpec, repoCheckout, events)
			case "container_start":
				containerID, opErr = d.Orchestrator.CreateAndStartWorkflowContainer(phaseCtx, image, job.Container.Entrypoint, repoCheckout, orchestratorVolumes(job.Container.Volumes, job.RepoID, d.AgentBinaryDir), events)
			}
		}()

		// Forward events as log lines with step_id = "phase:<kind>".
		phaseStepID := "phase:" + pd.kind
		for ev := range events {
			line := sanitizeLine(ev.Line)
			if line == "" {
				continue
			}
			d.appendLog(ctx, job.JobRunID, ev.Stream, line, phaseStepID)
		}
		phaseCancel()

		if opErr != nil {
			msg := fmt.Sprintf("phase %q: %v", pd.kind, opErr)
			d.appendSystemLog(ctx, job.JobRunID, msg)
			ec := int32(-1)
			if err := d.Client.TerminateWorkflowJobPhase(ctx, job.JobRunID, pd.kind, client.TerminatePhaseRequest{
				Status:   "failed",
				ExitCode: &ec,
				Message:  opErr.Error(),
			}); err != nil {
				log.Printf("workflow job %d: terminate phase %s: %v", job.JobRunID, pd.kind, err)
			}
	...
	for i, step := range job.Steps {
		...
		// Append exit code as a system log line attributed to this step so the
		// UI can surface it inside the step's log group.
		runLogStepID := step.ID
		if runLogStepID == "" {
			runLogStepID = fmt.Sprintf("%d", i+1)
		}
		d.appendLog(ctx, job.JobRunID, "system", fmt.Sprintf("exit code: %d", exitCode), runLogStepID)
...
func (d *WorkflowJobDriver) runStep(ctx context.Context, job *client.WorkflowJob, containerID, workingDir string, step client.WorkflowStep, stepIndex int) (int32, error) {
	...
	// Determine the step ID for log attribution. Use the step's explicit
	// ID, otherwise fall back to the 1-based step index.
	logStepID := step.ID
	if logStepID == "" {
		logStepID = fmt.Sprintf("%d", stepIndex+1)
	}
	...
	for drainers > 0 {
		select {
		case l := <-lines:
			d.appendLog(ctx, job.JobRunID, l.stream, l.line, logStepID)
		case err := <-errs:
			drainers--
			if err != nil {
...
// stepID identifies the currently executing step; empty for system-level
// log lines emitted between steps.
func (d *WorkflowJobDriver) appendLog(ctx context.Context, jobRunID int64, stream, line, stepID string) {
	if err := d.Client.AppendWorkflowJobLog(ctx, jobRunID, stream, line, stepID); err != nil {
		if ctx.Err() != nil {
			return
		}
...
//
// The reserved name "hangrix-agent" is special: instead of a named Docker
// volume, the runner emits a read-only bind mount of agentBinDir (the
// directory containing the embedded agent binary). See mapVolumes for the
// rationale. agentBinDir == "" disables the bind so the name falls back to
// a regular named volume.
func orchestratorVolumes(vols []client.Volume, repoID int64, agentBinDir string) []orchestrator.Volume {
	if len(vols) == 0 {
		return nil
	}
...
func releaseParamsFromWith(with map[string]any) (tag string, notes *string, draft *bool, assets []client.WorkflowStepAsset) {
	tag = withString(with, "tag")
	if raw, ok := with["notes"].(string); ok && raw != "" {
		notes = &raw
	}
...
func (d *WorkflowJobDriver) runReleaseStep(
	...
	// Determine the log step ID for attributing release log lines to this step.
	logStepID := step.ID
	if logStepID == "" {
		logStepID = fmt.Sprintf("%d", stepIndex+1)
	}
	...
	if notes != nil {
		expanded, err := expandStepOutputRefs(*notes, stepOutputs)
		if err != nil {
	...
	d.appendLog(ctx, job.JobRunID, "system", fmt.Sprintf("creating release %q", expandedTag), logStepID)
	...
	for _, a := range expandedAssets {
		...
		d.appendLog(ctx, job.JobRunID, "system", fmt.Sprintf("uploading asset %q from %s", assetName, a.Path), logStepID)
	...
	published := false
	if !draft {
		d.appendLog(ctx, job.JobRunID, "system", fmt.Sprintf("publishing release %d", rel.ID), logStepID)
...
func (d *WorkflowJobDriver) runScriptStep(
	...
	// 5. Determine the step ID for log attribution and build the
	//    scriptbridge driver.
	logStepID := step.ID
	if logStepID == "" {
		logStepID = fmt.Sprintf("%d", stepIndex+1)
	...
	sd := &scriptbridge.Driver{
		...
		Log: func(stream, line string) {
			d.appendLog(ctx, job.JobRunID, stream, line, logStepID)
		},
	}
...
// ansiRe matches ANSI escape sequences (color codes, cursor movements).
var ansiRe = regexp.MustCompile(`\x1b\[[0-9;]*[a-zA-Z]`)

// sanitizeLine cleans a line of docker output: truncates \r-delimited
// progress bars to the last segment and strips ANSI escape sequences.
// Returns the original line unchanged when no sanitization is needed.
func sanitizeLine(line string) string {
	// Fast path: nothing to clean.
	if !strings.ContainsRune(line, '\r') && !strings.ContainsRune(line, '\x1b') {
		return line
	}

	// Split on \r and take the last non-empty segment after ANSI cleanup.
	parts := strings.Split(line, "\r")
	// Walk backwards to find the last non-empty meaningful segment.
	for i := len(parts) - 1; i >= 0; i-- {
		p := strings.TrimSpace(parts[i])
		if p == "" {
			continue
		}
		p = ansiRe.ReplaceAllString(p, "")
