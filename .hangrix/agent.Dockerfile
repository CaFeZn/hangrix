...
# Keep the Playwright CLI installed in the image and store browsers in a shared
# path instead of a per-user cache. `playwright install --with-deps chromium`
# installs the Linux packages Chromium needs and downloads the Chrome for
# Testing binary plus headless shell that Playwright drives.
