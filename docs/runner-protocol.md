...
## Cause event — issue.opened payload

当服务端因为 `issue.opened` 触发 agent session 时，会向 agent stdin 投递 `kind: event` 帧：
...
```json
{
  ...
  "payload": {
    ...
    "cause_id": "",
    "issue_title": "Bug: login flow broken",
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `repo_id` | number | 触发事件所在仓库 ID |
| `issue_number` | number | 触发事件的 issue 编号 |
| `actor_id` | number | 触发者用户 ID（issue.opened 场景下为创建者） |
| `cause_kind` | string | cause 类型，`issue.opened` 对应 `issue_opened` |
| `cause_id` | string | 上游 artefact ID；`issue.opened` 固定为空字符串 |
| `issue_title` | string | 新建 issue 的标题 |
| `issue_body` | string | 新建 issue 的正文（原样传递，不截断、不改写） |

说明：`issue_title` / `issue_body` 由触发方通过 `TriggerInput.Payload` 追加；若构造 payload 时 JSON 编码失败，服务端会静默退化为仅发送基础字段，不阻塞 issue 创建。
